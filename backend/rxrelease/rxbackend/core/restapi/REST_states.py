...
class REST_states(REST_base):
    ...
    def putSimpleState(self, simple_state):
        serverAddress = self.backendlocation + '/rxbackend/simplestates/' + str(
            simple_state['id']) + '/'
        headers = {'content-type': 'application/json'}
        headers.update(self.getAuthTokenHeader())
    ...
    def putRepeatableState(self, repeatable_state):
        serverAddress = self.backendlocation + '/rxbackend/repeatablestates/' + str(
            repeatable_state['id']) + '/'
        headers = {'content-type': 'application/json'}
        headers.update(self.getAuthTokenHeader())
    ...
    def putComplexState(self, complex_state):
        serverAddress = self.backendlocation + '/rxbackend/complexstates/' + str(
        complex_state['id']) + '/'
        headers = {'content-type': 'application/json'}
        headers.update(self.getAuthTokenHeader())
        response = requests.put(serverAddress, data=json.dumps(complex_state), headers=headers)
        return response
    ...
    def getStateByHostAndStateTypeId(self, host_id, statetype_id):
        serverAddress = self.backendlocation + '/rxbackend/states/search/?host_id=' + str(
            host_id) + "&" + "statetype_id=" + str(statetype_id)
