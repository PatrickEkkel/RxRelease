...
class SettingsDao:
    ...
    def get_credentials_by_category_id(self,category_id):
        kvsettings = CredentialsSetting.objects.get(category_id=category_id)
        return kvsettings
    ...
    def getCredentialsByCategory(self, category):
        try:
            ...
            kvsettings = None
            logger.error(
                "Trying to get Settings by a Category that does not exists with name " + str(
                    category))
        return kvsettings
